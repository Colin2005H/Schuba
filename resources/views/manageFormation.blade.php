...
<script>
    ...
    function loadManagersN1(id) {
        ...
        if (id == "") {
            selectedN1 = "";
            document.querySelectorAll("form #manager-select option").forEach(function(elt) {
                elt.disabled = false;
            })
            if (selectedN2 != "") {
...
@section('content')
    ...
    <div class="bg-[url('/img/scub1.png')] h-full bg-cover bg-no-repeat bg-center p-10">
        <div class="bg-[rgb(255,255,255,0.8)] px-16 py-10 m-10 justify-items-center rounded">
            ...
            <div class="flex">
                <form action="/formations" method="post"
                    class="mx-4 my-6 max-w-96 flex flex-col border-2 border-[#004B85] rounded">
                    @csrf

                    <div class="flex m-3">
                        <label for="category-select" class="font-bold mr-6 text-xl">Niveau&nbsp;:&nbsp;N1</label>
                        <select name="category" id="category-select" class="rounded max-w-64 drop-shadow-lg hidden"
                            required>
                    ...
                    <div class="flex m-3">
                        ...
                        <select name="manager" id="manager-select" class="rounded max-w-64 drop-shadow-lg" required
                            onchange="loadManagersN1(this.selectedOptions[0].value)">
                            <option value="">-- Sélectionnez une option --</option>
                            @foreach ($optionsManager as $option)
                                <option @if ($n1Exist === 1 && $n1ExistManager->first()->UTI_ID == $option->UTI_ID) selected @endif id="n1_{{ $option->UTI_ID }}"
                                    value="{{ $option->UTI_ID }}">{{ $option->UTI_NOM }} {{ $option->UTI_PRENOM }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex m-3">
                        ...
                        <select name="initiator[]" id="initiator-select" multiple="multiple" required
                            class="rounded max-w-64 drop-shadow-lg max-h-12 min-w-48">
                            @php
                                $existingInitiatorIdsN1 = collect($n1ExistInitiators)->pluck('UTI_ID')->toArray();
                            @endphp
                            @foreach ($optionsInitiatorsN1N2 as $option)
                                <option @if ($n1Exist === 1 && in_array($option->UTI_ID, $existingInitiatorIdsN1)) selected @endif value="{{ $option->UTI_ID }}">
                                    {{ $option->UTI_NOM }} {{ $option->UTI_PRENOM }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex m-3">
                        ...
                        <select name="student[]" id="student-select" multiple="multiple" required
                            ...
                            @foreach ($optionsStudentN1 as $option)
                                <option @if ($n1Exist === 1 && in_array($option->UTI_ID, $existingStudentIdsN1)) selected @endif value="{{ $option->UTI_ID }}">
                                    {{ $option->UTI_NOM }} {{ $option->UTI_PRENOM }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    @if ($n1Exist === 0)
                        <div class="text-center m-4">
                            <input type="submit" value="Créer" id="btnCreateN1"
                                class="bg-[#000000] py-2 px-4 rounded text-white" />
                        </div>
                    @else
                        <div class="text-center m-4">
                            <input type="submit" value="Modifier" id="btnModifyN1"
                                class="bg-[#000000] py-2 px-4 rounded text-white" />
                        </div>
                    @endif
                    ...
                    @if ($n1NbSessions === 0 && $n1Exist === 1)
                        <input type="submit" name="action" value="Supprimer"
                            class="bg-[#000000] py-2 px-3 mb-4 self-center rounded text-white " />
                    @endif
                ...
                <form action="/formations" method="post"
                    ...
                    <div class="flex m-3">
                        <label for="manager-select" class="font-bold mr-6">Responsable&nbsp;:&nbsp;</label>
                        <select name="manager" id="manager-select" class="rounded max-w-64 drop-shadow-lg" required
                            onchange="loadManagersN2(this.selectedOptions[0].value)">
                            <option value="">-- Sélectionnez une option --</option>
                            @foreach ($optionsManager as $option)
                                <option @if ($n2Exist === 1 && $n2ExistManager->first()->UTI_ID == $option->UTI_ID) selected @endif value="{{ $option->UTI_ID }}"
                                    id="n2_{{ $option->UTI_ID }}">{{ $option->UTI_NOM }} {{ $option->UTI_PRENOM }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex m-3">
                        ...
                        <select name="initiator[]" id="initiator-select" multiple="multiple" required
                            ...
                            @endphp
                            @foreach ($optionsInitiatorsN1N2 as $option)
                                <option @if ($n2Exist === 1 && in_array($option->UTI_ID, $existingInitiatorIdsN2)) selected @endif value="{{ $option->UTI_ID }}">
                                    {{ $option->UTI_NOM }} {{ $option->UTI_PRENOM }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex m-3">
                        ...
                        <select name="student[]" id="student-select" multiple="multiple" required
                            ...
                            @foreach ($optionsStudentN2 as $option)
                                <option @if ($n2Exist === 1 && in_array($option->UTI_ID, $existingStudentIdsN2)) selected @endif value="{{ $option->UTI_ID }}">
                                    {{ $option->UTI_NOM }} {{ $option->UTI_PRENOM }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    @if ($n2Exist === 0)
                        <div class="text-center m-4">
                            <input type="submit" value="Créer" id="btnCreateN2" onclick="createdN2()"
                                class="bg-[#000000] py-2 px-4 rounded text-white" />
                        </div>
                    @else
                        <div class="text-center m-4">
                            <input type="submit" value="Modifier" id="btnModifyN2"
                                class="bg-[#000000] py-2 px-4 rounded text-white" />
                        </div>
                    @endif
                ...
                <form action="/formations" method="post"
                    ...
                    <div class="flex m-3">
                        <label for="manager-select" class="font-bold mr-6">Responsable&nbsp;:&nbsp;</label>
                        <select name="manager" id="manager-select" class="rounded max-w-64 drop-shadow-lg" required
                            onchange="loadManagersN3(this.selectedOptions[0].value)">
                            <option value="">-- Sélectionnez une option --</option>
                            @foreach ($optionsManager as $option)
                                <option @if ($n3Exist === 1 && $n3ExistManager->first()->UTI_ID == $option->UTI_ID) selected @endif id="n3_{{ $option->UTI_ID }}"
                                    value="{{ $option->UTI_ID }}">{{ $option->UTI_NOM }} {{ $option->UTI_PRENOM }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex m-3">
                        <label for="initiator-select"
                            class="font-bold content-center mr-6">Initiateurs&nbsp;:&nbsp;</label>
                        <select name="initiator[]" id="initiator-select" multiple="multiple" required
                            class="rounded max-w-64 drop-shadow-lg max-h-12 min-w-48">
                            @php
                            ...
                            @foreach ($optionsInitiatorsN3 as $option)
                                <option @if ($n3Exist === 1 && in_array($option->UTI_ID, $existingInitiatorIdsN3)) selected @endif value="{{ $option->UTI_ID }}">
                                    {{ $option->UTI_NOM }} {{ $option->UTI_PRENOM }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex m-3">
                        ...
                        <select name="student[]" id="student-select" multiple="multiple" required
                            ...
                            @foreach ($optionsStudentN3 as $option)
                                <option @if ($n3Exist === 1 && in_array($option->UTI_ID, $existingStudentIdsN3)) selected @endif value="{{ $option->UTI_ID }}">
                                    {{ $option->UTI_NOM }} {{ $option->UTI_PRENOM }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    @if ($n3Exist === 0)
                        <div class="text-center m-4">
                            <input type="submit" value="Créer" id="btnCreateN3" onclick="createdN3()"
                                class="bg-[#000000] py-2 px-4 rounded text-white" />
                        </div>
                    @else
                        <div class="text-center m-4">
                            <input type="submit" value="Modifier" id="btnModifyN3"
                                class="bg-[#000000] py-2 px-4 rounded text-white" />
                        </div>
                    @endif
