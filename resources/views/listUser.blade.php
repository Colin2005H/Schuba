            ...
            <tbody>
               @foreach ($liste_eleve as $item)
                @if($item->UTI_SUPPRIME==0)
                        ...
                        @csrf
                        <td class="text-center py-4 px-6"><button name="action" value="{{$item->UTI_ID}}" class="bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-md shadow-md focus:outline-none focus:ring-2 focus:ring-red-300" onclick="submitDelete()" href ="modifier" >Supprimer</button></td>
                        </form>
                    </tr>
